...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, config):
        ...
        out_prob = outputs["pred_logits"].softmax(-1)  # [batch_size, num_queries, num_classes][8,156,125]
        out_kpt = outputs["pred_coords"]  # [batch_size, num_queries, 2][8,156,2]
        ...
        cost_class = -out_prob[..., landmark_index] # [B, num_queries, num_joints]
        cost_class = cost_class[..., :-1]
        # cost_class = -out_prob[..., :num_joints]

        # Compute the L1 cost between keypoints
        cost_kpt = torch.cdist(out_kpt, targets, p=1)  # [B, num_queries, num_joints]
        ...
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices], num_joints, landmark_index
